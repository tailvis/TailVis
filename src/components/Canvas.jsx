...
// Canvas chrome — selection boxes, lineage connectors, resize handles — is UI, not
// content: it must keep the same on-screen size at every zoom level. Strokes get
// vector-effect="non-scaling-stroke", which keeps both the width and the dash pattern
// in screen pixels for free. Shapes that a stroke hint can't fix (arrowheads, handle
// rects) are counter-scaled by 1/k in refreshChromeScale below, since zooming only
// re-applies the main group's transform and never redraws these.
// All widths/sizes below are on-screen px and stay put at any zoom — tune here.
const NON_SCALING = 'non-scaling-stroke'
const CHROME_STROKE_WIDTH = 1          // selection box, resize handle outline
const CHROME_HIGHLIGHT_WIDTH = 2       // connected-chart dashed border
const CONNECTOR_WIDTH = 1              // lineage connector, idle
const CONNECTOR_WIDTH_ACTIVE = 2       // lineage connector, in the highlighted chain
const ARROW_LEN = 6                    // arrowhead length / half-width, idle
const ARROW_HALF = 3.5
const ARROW_LEN_ACTIVE = 9             // …and in the highlighted chain
const ARROW_HALF_ACTIVE = 5
const HANDLE_SIZE = 8
...
function Canvas({
  ...
  // Place a chrome arrowhead: its points are authored around the origin at screen
  // scale, so the transform carries both the anchor and the 1/k counter-scale.
  const chromeArrowTransform = useCallback((ax, ay) => (
    `translate(${ax}, ${ay}) scale(${1 / transformRef.current.k})`
  ), [])

  // Re-fit the chrome that can't ride on non-scaling-stroke after a zoom.
  const refreshChromeScale = useCallback((k) => {
    const svg = svgRef.current
    if (!svg) return
    svg.querySelectorAll('.chrome-arrow').forEach(el => {
      el.setAttribute('transform', `translate(${el.getAttribute('data-ax')}, ${el.getAttribute('data-ay')}) scale(${1 / k})`)
    })
    const size = HANDLE_SIZE / k
    const half = size / 2
    svg.querySelectorAll('.chart-group').forEach(group => {
      const handles = group.querySelectorAll('.resize-handle')
      if (!handles.length) return
      const bg = group.querySelector('.chart-background')
      const w = parseFloat(bg?.getAttribute('width'))
      const h = parseFloat(bg?.getAttribute('height'))
      if (!isFinite(w) || !isFinite(h)) return
      handles.forEach(el => {
        el.setAttribute('x', w * parseFloat(el.getAttribute('data-xr')) - half)
        el.setAttribute('y', h * parseFloat(el.getAttribute('data-yr')) - half)
  ...
  useEffect(() => { refreshChromeScale(transform.k) }, [transform.k, refreshChromeScale])
      ...
      .on('drag', function(event) {
        ...
        if (!marquee) {
          marquee = mainGroup.append('rect')
            ...
            .attr('stroke', '#4a9eff')
            .attr('stroke-width', CHROME_STROKE_WIDTH)
            .attr('vector-effect', NON_SCALING)
    ...
    chartObjects.forEach(chartObj => {
      if (chartObj.parentId) {
        ...
        if (parentChart) {
          ...
          const lineWidth = isConnectionHighlighted ? CONNECTOR_WIDTH_ACTIVE : CONNECTOR_WIDTH

          const lineAttrs = { fill: 'none', stroke: lineColor, 'stroke-width': lineWidth, 'vector-effect': NON_SCALING, 'stroke-dasharray': isConnectionHighlighted ? 'none' : '4,4', opacity: lineOpacity }
          ...
          const as = isConnectionHighlighted ? ARROW_LEN_ACTIVE : ARROW_LEN
          const ah = isConnectionHighlighted ? ARROW_HALF_ACTIVE : ARROW_HALF
          arrowPoints = isDataTransform
            ? `0,0 ${-ah},${-as} ${ah},${-as}`
            : `0,0 ${-as},${-ah} ${-as},${ah}`
          connectionsGroup.append('polygon')
            .attr('class', 'chrome-arrow')
            .attr('points', arrowPoints)
            .attr('data-ax', endX)
            .attr('data-ay', endY)
            .attr('transform', chromeArrowTransform(endX, endY))
            .attr('fill', lineColor)
    ...
    chartObjects.forEach(chartObj => {
      ...
      if (isConnected && !isSelected) {
        chartGroup.append('rect')
          ...
          .attr('fill', 'none')
          .attr('stroke', '#4a9eff')
          .attr('stroke-width', CHROME_HIGHLIGHT_WIDTH)
          .attr('vector-effect', NON_SCALING)
      ...
      // Figma-style resize handles (only for single selection; never in read-only
      // gallery — resizing rewrites the spec, which read-only mode rejects).
      if (isSingleSelected && !readOnly) {
    ...
    if (previewChart) {
      ...
      if (sourceChart) {
        ...
        previewGroup.append('path')
          ...
          .attr('fill', 'none')
          .attr('stroke', '#4a9eff')
          .attr('stroke-width', CONNECTOR_WIDTH_ACTIVE)
          .attr('vector-effect', NON_SCALING)
          .attr('stroke-dasharray', '8,4')
          .attr('opacity', 0.8)

        // Arrow
        const ax = endX - previewChart.x
        const ay = endY - previewChart.y
        previewGroup.append('polygon')
          .attr('class', 'chrome-arrow')
          .attr('points', `0,0 ${-ARROW_LEN_ACTIVE},${-ARROW_HALF_ACTIVE} ${-ARROW_LEN_ACTIVE},${ARROW_HALF_ACTIVE}`)
          .attr('data-ax', ax)
          .attr('data-ay', ay)
          .attr('transform', chromeArrowTransform(ax, ay))
          .attr('fill', '#4a9eff')
          .attr('opacity', 0.8)
      }
    }
  ...
  }, [chartObjects, onSelectChart, onUpdateChart, previewChart, widgetOffsets, readOnly])
  ...
  useEffect(() => {
    ...
    if (connectionsGroup) {
      ...
      chartObjects.forEach(chartObj => {
        ...
        const as = isConnectionHighlighted ? ARROW_LEN_ACTIVE : ARROW_LEN
        const ah = isConnectionHighlighted ? ARROW_HALF_ACTIVE : ARROW_HALF
        const arrowPoints = isDataTransform
          ? `0,0 ${-ah},${-as} ${ah},${-as}`
          : `0,0 ${-as},${-ah} ${-as},${ah}`
    ...
    chartObjects.forEach(chartObj => {
      ...
      if (isSelected && !existingBox) {
        ...
        box.setAttribute('stroke-width', CHROME_STROKE_WIDTH)
        box.setAttribute('vector-effect', NON_SCALING)
      ...
      if (isConnected && !isSelected && !existingConnected) {
        ...
        hl.setAttribute('stroke-width', CHROME_HIGHLIGHT_WIDTH)
        hl.setAttribute('vector-effect', NON_SCALING)
      ...
      if (isSingleSelected && existingHandles.length === 0 && !readOnly) {
  ...
  }, [selectedChartIds, chartObjects, onUpdateChart, readOnly])
  ...
  return (
    <div className="canvas-container">
      ...
      {/* + Button overlay — above widgets. Hidden in the read-only gallery: it only
          opens the chat to author a new derived chart, which read-only mode disallows. */}
      <div className="add-button-overlay">
        {(readOnly ? [] : chartObjects).map(chartObj => {
