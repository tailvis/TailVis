...
function App({ readOnly = false, galleryExample = null } = {}) {
  ...
  const handleUpdateChart = useCallback((id, updates) => {
    // Read-only gallery: block content edits, but ALLOW pure layout updates —
    // size measurements from the renderer (onSizeReady) and canvas positions from a
    // chart drag. Both are view-only arrangement (never persisted, see saveToStorage),
    // and letting x/y through keeps dragging identical to the editor: the chart, its
    // parent-child arrows, and the widget overlays all move together.
    if (readOnly) {
      const keys = Object.keys(updates || {})
      const layoutOnly = keys.length > 0 && keys.every(k => k === 'width' || k === 'height' || k === 'x' || k === 'y')
